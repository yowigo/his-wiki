...
#### 2026\-05\-14 V3.9.7     
\* UISymbolPanel: 选中图标时，自动复制到剪贴板    
\* UIForm: 调整窗体右上角控制按钮的绘制区域    
\* UITrackBar: 增加进度条显示方式（从起点到终点、从终点到起点、从中间到边缘、不显示）    
\* UIFormHelper: 修复窗体最小化时，弹窗报错 [#IJL13C](https://gitee.com/yhuse/SunnyUI/issues/IJL13C)    
\- UIGifAvatar: 删除 UIGifAvatar，可从 [获取源码](https://gitee.com/yhuse/SunnyUI/blob/V3.9.6/SunnyUI/Controls/UIGifAvatar.cs)     
\* GraphicsEx: 缓存了常用的Pen和Brush对象，避免重复创建    
    
#### 2026\-04\-16 V3.9.6     
\* UIComboBoxItem: 修改下拉框自定义颜色    
\* UINavBar: 更新属性编辑器      
\* UITreeView: 更新属性编辑器    
\* UIRadiusSidesEditor: 重构圆角属性编辑器    
\* UIControl: 使用默认的设计期判断      
    
#### 2026\-04\-10 V3.9.5    
\+ UIStyle: 增加一个深色主题Dark    
\* UINavMenu: 增加节点右侧文本显示功能    
\* UINavMenu: 增加节点展开箭头位置属性，可以设置箭头显示在左侧    
\* UIContextMenuStrip: 更新构造函数    
    
#### 2026\-04\-01 V3.9.4    
\+ UIUpDownTextBox: 增加一个含有上下按钮的输入框    
\* UIDataGridView: 修复了一个SelectedIndex属性读写可能不一致的问题    
    
#### 2026\-03\-12 V3.9.3    
\* TranslateHelper: 多语言生成窗体控件多语翻译资源时，未能生成放在TableLayoutPanel里面的控件的多语翻译资源  [#IF21LU](https://gitee.com/yhuse/SunnyUI/issues/IF21LU)     
\* UIMeter: 增加刻度值小数位数自适应    
    
#### 2026\-01\-31 V3.9.2           
\* UITabControl: 页面移除时响应 FormClose    
\* UIForm: 修复窗体调整大小的Bug， [#IDHCVP](https://gitee.com/yhuse/SunnyUI/issues/IDHCVP)      
    
#### 2025\-12\-20 V3.9.1           
\* UIStatusForm: 优化多行文本显示时的布局调整，支持任意行数的文本内容显示。    
\* UIDataGridView: 修改行头部颜色    
\* UIPage: TitleFillColor颜色设置问题，临时增加了一个事件 TitlePainting    
\* UIForm: 修改无边框窗体默认最大化的方式    
\* UIColorPicker: 增加 HideDropDown 方法    
\* UIComboDataGridView: 增加 HideDropDown 方法    
\* UIComboTreeView: 增加 HideDropDown 方法    
\* UIDatePicker: 增加 HideDropDown 方法    
\* UIDatetimePicker: 增加 HideDropDown 方法    
\* UITimePicker: 增加 HideDropDown 方法    
\* UILoginForm: 增加可自定义背景图片    
    
#### 2025\-11\-16 V3.9.0              
\+ 增加 .NET10.0 支持    
\* Dialogs: 增加文件多选的打开对话框    
\* UIDatetimePicker: 优化加载速度 [#ID2PNR](https://gitee.com/yhuse/SunnyUI/issues/ID2PNR)    
\* UIDropControl: 优化一些下拉控件的加载速度    
\* UIDatePicker: 优化日期转换方法，增加区域信息    
\* UIDateTimePicker: 优化日期转换方法，增加区域信息    
\* UITimePicker: 优化日期转换方法，增加区域信息    
\* UIPage: 修复TitleFillColor颜色设置问题 [#ID4MEF](https://gitee.com/yhuse/SunnyUI/issues/ID4MEF)    
    
#### 2025\-10\-16 V3.8.9          
\* UIDropControl: 修复设置设置圆弧形时，右侧显示有问题 [#I95DY6](https://gitee.com/yhuse/SunnyUI/issues/I95DY6)    
\* UIWaitForm: 等待提示框增加取消功能 [#IA5Q0Z](https://gitee.com/yhuse/SunnyUI/issues/IA5Q0Z)    
\* UIEditForm: 代码生成增加多行文本功能 [#IB7C0O](https://gitee.com/yhuse/SunnyUI/issues/IB7C0O)    
\* UIStyles: 修复一个复杂情况下切换主题时的异常 [#IC24XE](https://gitee.com/yhuse/SunnyUI/issues/IC24XE)    
\* UIDatetimePicker: 添加MaxDate与MinDate属性 [#IC6HOJ](https://gitee.com/yhuse/SunnyUI/issues/IC6HOJ)    
\* UTranslate: V3.8.8 多语言支持 .NET8 窗体项目 [#IBVYE1](https://gitee.com/yhuse/SunnyUI/issues/IBVYE1)    
\* UIFormHelper: 增加 YesNoCancel 弹窗 [#IBFFN1](https://gitee.com/yhuse/SunnyUI/issues/IBFFN1)     
\* 更新翻译     
\* 更改窗体图标绘制方法     
\* 多彩主题: 更新随机配色算法    
...
\* UILineChart: 修改了X轴显示为文字时，鼠标移上显示数据点标签    
\* UILedStopwatch: 修改小时显示，可超过24小时显示 [#ICUFNY](https://gitee.com/yhuse/SunnyUI/issues/ICUFNY)     
\* UIIPTextBox: 增加快捷键 CTRL+CV 复制和粘贴IP地址 [#ICUF80](https://gitee.com/yhuse/SunnyUI/issues/ICUF80)     
\* UIMessageForm2: 使用TextRenderer绘制文本（避免削顶问题）    
\* UIEdit: 水印绘制方法恢复至上一版本 [#ICX0O0](https://gitee.com/yhuse/SunnyUI/issues/ICX0O0)     
    
#### 2025\-07\-27 V3.8.7            
\+ UIScratchCode: 刮刮卡类型验证码控件    
\* UIniFileEx: 增加对 .NetFramework 的支持    
\* UILineChart: 解决一个日期显示丢失精度的问题    
\* UIPage: 调整标题栏 ExtendMenu 显示位置，[#ICL933](https://gitee.com/yhuse/SunnyUI/issues/ICL933)        
\* UIForm: 调整标题栏 ExtendMenu 显示位置    
\* UIForm2: 调整标题栏 ExtendMenu 显示位置    
\* UIEdit: 重写水印文字的绘制逻辑    
\* UITextBox: 调整编辑框显示文字位置    
\* UIVerificationCode: 双击修改验证码，验证码采用Base32字母表，更容易识别    
    
#### 2025\-07\-05 V3.8.6       
\*  **重构，删除FileEx, DirEx, 对话框扩展类调整至 Dialogs**     
\* UINumPadTextBox: 无需设置DropDownStyle属性    
\* UINumPadTextBox: 增加文本对齐方式    
\* UINumPadTextBox: 增加弹窗长按删除按钮，清空文本框    
\* UIDoubleUpDown: 增加两侧按钮长按快速增加或减少值    
\* UIIntegerUpDown: 增加两侧按钮长按快速增加或减少值    
\* UICheckBoxGroup: 修复索引赋值为空时出错的问题    
\* UIRadioButtonGroup: 增加鼠标滑过填充颜色 [#ICFYK4](https://gitee.com/yhuse/SunnyUI/issues/ICFYK4)     
    
#### 2025\-06\-16 V3.8.5    
\* UIDoubleUpDown: 重写控件    
\* UIIntegerUpDown: 重写控件    
\* UIVerificationCode: 修复验证码字符相同 [#ICBL2X](https://gitee.com/yhuse/SunnyUI/issues/ICBL2X)     
\* UILineChartOption: 增加了鼠标漫游时按所有点查找数据点的方式，数据点多时慎用 [#ICCKND](https://gitee.com/yhuse/SunnyUI/issues/ICCKND)     
\* UIEdit: 多行时水印文字在左上角显示    
\* UITabControl: 修复Close按钮点击无法关闭标签 [#ICEIHR](https://gitee.com/yhuse/SunnyUI/issues/ICEIHR)    
    
#### 2025\-05\-28 V3.8.4     
\+ UIMeter: 增加仪表控件    
\- UIAnalogMeter: 删除UIAnalogMeter控件，源码见 [UIAnalogMeter](https://gitee.com/yhuse/SunnyUI/blob/V3.8.3/SunnyUI/Controls/UIAnalogMeter.cs)
\* UINavMenu: 增加节点文字居中的属性    
\* UITabControl: 修复不可关闭的主页被关闭了 [#IC1XIU](https://gitee.com/yhuse/SunnyUI/issues/IC1XIU)      
\* UITabControl: 更新部分注释    
\* UIDataGridView: 修复数据更新时是否自动滚动到最后一行    
\* UIForm: 几个提示框增加了是否屏幕居中的属性，默认True，Falsse则为父窗体居中 [#IC6CAZ](https://gitee.com/yhuse/SunnyUI/issues/IC6CAZ)      
\* UIComboBox: 非过滤且显示为列表时，增加通过鼠标滚轮选择上下行    
\* UIComboBoxItem: 修改下拉框自定义颜色    
...
\* UIDigitalLabel: 增加ShowText属性，可以显示文字 [#IBY64V](https://gitee.com/yhuse/SunnyUI/issues/IBY64V)     
\* UIBreadcrumb: 增加SetItemColor，设置单个节点颜色，可以用RemoveItemColor移除    
\* EnumEx: DisplayText特性改为原生的Description特性    
\* 更新一些控件的字体图标    
\* 增加控件在工具箱说明文字    
...
\* UITabControl: 增加标签页分割线属性ShowTabDivider，[#IBLERL](https://gitee.com/yhuse/SunnyUI/issues/IBLERL)     
\* UITabControlMenu: 修复切换主题色时，TabPage未设置背景色，[#IBKDR7](https://gitee.com/yhuse/SunnyUI/issues/IBKDR7)        
\* UITabControl: 修复切换主题色时，TabPage未设置背景色，[#IBKDR7](https://gitee.com/yhuse/SunnyUI/issues/IBKDR7)          
\* UIColorPicker: 增加手动输入ARGB值调色 [#IBHL0H](https://gitee.com/yhuse/SunnyUI/issues/IBHL0H)              
\* UIMenuButton: 更改属性描述    
\* UIForm: 修复窗体边框显示不全 [#IBGJBS](https://gitee.com/yhuse/SunnyUI/issues/IBGJBS)     
\* UFontAwesomeV6: 更新为Font Awesome version: 6.7.2    
    
#### 2024\-12\-27 V3.8.1    
\- 移除.Net6 支持，建议直接升级.Net8    
\+ UIMenuButton: 增加下拉菜单按钮    
\* UFontAwesomeV6: 更新为Font Awesome version: 6.7.1    
\* UITitlePanel: 增加Padding设置 [#IBB1UF](https://gitee.com/yhuse/SunnyUI/issues/IBB1UF)            
\* UITabControl: 增加未选页签颜色 [#IB7U69](https://gitee.com/yhuse/SunnyUI/issues/IB7U69)            
\* UITabControl: 修复标签文字越界显示 [#IB8571](https://gitee.com/yhuse/SunnyUI/issues/IB8571)            
\* UIVerificationCode: 可以自定义颜色 [#IBABW1](https://gitee.com/yhuse/SunnyUI/issues/IBABW1)            
\* UIComboDataGridView: 修复一个报错 [#IB8YK9](https://gitee.com/yhuse/SunnyUI/issues/IB8YK9)            
\* UICalendar: 切换语言实时刷新    
\* UIScrollingText: 停止滚动时，可以设置默认显示位置    
\* UICheckBoxGroup: 修复TitleTop为0时，条目显示错位的问题 [#IB7STO](https://gitee.com/yhuse/SunnyUI/issues/IB7STO)            
\* UITabControl: 修复了SelectedIndex=-1时的报错    
\* UIComboDataGridView: 解决下拉框显示过滤编辑框不能一直显示 [#IB7AFB](https://gitee.com/yhuse/SunnyUI/issues/IB7AFB)            
    
#### 2024\-11\-19 V3.8.0    
\+ 增加.Net9 支持    
\* UIDigitalLabel: 增加DPI支持    
\* UIBarChart: 增加了一个快捷的显示数据函数ShowData    
\* UIComboBox: 增加了SelectionChangeCommitted事件，下拉框显示鼠标点击条目时响应 [#IB064Z](https://gitee.com/yhuse/SunnyUI/issues/IB064Z)            
\* UIBaseForm: IconImage增加IconImageSize属性，可调整大小    
\* UIPage: 增加标题栏图片属性IconImage，优先于Symbol    
\* UIScrollBar: 修复了一些重载事件 [#IB1UV7](https://gitee.com/yhuse/SunnyUI/issues/IB1UV7)        
\* UControl: UITreeView增加了一个刷新节点状态的扩展函数    
...
\* UIComboDataGridView: 增加下拉框滚动条宽度调整属性    
\* UILineChart: 增加了鼠标移上绘制十字线样式    
\* UIPanel: 增加透明度    
\* UINotifier: 点击提示关闭，增加输出提示    
      
#### 2024\-10\-06 V3.7.2    
\* **重构多语翻译类，详细文档见：[国际化](https://gitee.com/yhuse/SunnyUI/wikis/%E5%9B%BD%E9%99%85%E5%8C%96)**    
\* 多页面框架重构    
\* UIDoubleUpDown: 增加加按钮字体图标的偏移位置    
\* UIDataGridView: 增加属性AutoScrollToBottom，数据更新时是否自动滚动到最后一行    
\* UIDatePicker: 修复格式化字符串包含/时显示错误    
\* UIDateTimePicker: 修复格式化字符串包含/时显示错误    
\* UIRadioButton: 修改AutoSize属性可以保存在Design.cs文件里面，[#IAKYX4](https://gitee.com/yhuse/SunnyUI/issues/IAKYX4)        
\* UICheckBox: 修改AutoSize属性可以保存在Design.cs文件里面，[#IAKYX4](https://gitee.com/yhuse/SunnyUI/issues/IAKYX4)        
\* UINumPadTextBox: 增加ShowDropDown()弹窗方法    
\* UIDataGridView: 解决有隐藏行时，滚动条滚动时出错的问题 [#IAOW3A](https://gitee.com/yhuse/SunnyUI/issues/IAOW3A)       
\* UICheckBoxGroup: 更改计算节点位置的方法，解决问题：[#IAPY94](https://gitee.com/yhuse/SunnyUI/issues/IAPY94)       
\* UIRadioButtonGroup: 更改计算节点位置的方法，解决问题：[#IAPY94](https://gitee.com/yhuse/SunnyUI/issues/IAPY94)      
